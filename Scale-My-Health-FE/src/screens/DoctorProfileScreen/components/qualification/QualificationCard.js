...
import { EditQualificationAPI, RemoveQualificationAPI } from '../../../../api/utils'
...
const QualificationCard = ({
    ...
    const onSavePress = async (data) => {
        try {
            const res = await EditQualificationAPI(doctorId, data);
            if (res?.data?.status) {
    ...
    const onRemovePress = async (data) => {
        try {
            const res = await RemoveQualificationAPI(doctorId, data);
            if (res?.data?.status) {
                const data = res?.data?.data;
                if (data != null) {
                    dispatch(setUserDataInRedux(data));
    ...
    return (
        <>
            ...
            <ConfirmationModal
                title={data?.type}
                desc={`Are you sure you want to remove this qualification?`}
